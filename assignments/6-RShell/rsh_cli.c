...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        if (strcmp(cmd_buff, EXIT_CMD) == 0) {
            // Send exit command to server to close the connection properly
        ...
        // Send command to server (null-terminated)
        io_size = send(cli_socket, cmd_buff, strlen(cmd_buff) + 1, 0);
        if (io_size < 0) {
            perror("send");
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        ...
        // Receive and display responses until EOF character is received
        while (1) {
            ssize_t recv_bytes = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ - 1, 0);
            ...
            // Print received data (may not be null-terminated)
            if (is_eof) {
                // Print all except the EOF character
                printf("%.*s", (int)recv_bytes - 1, rsp_buff);
            } else {
                printf("%.*s", (int)recv_bytes, rsp_buff);
            }
            
            // If EOF received, break out of the receive loop
            if (is_eof) {
                break;
            }
        }
